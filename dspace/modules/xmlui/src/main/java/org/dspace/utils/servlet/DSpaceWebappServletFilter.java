...
/**
 * This servlet filter will handle the hookup and setup for DSpace requests. It
 * should be applied to any webapp that is using the DSpace core.
 * <p>
 * It will also do anything necessary to the requests that are coming into a
 * DSpace web application and the responses on their way out.
 * 
...
public final class DSpaceWebappServletFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (pathShouldBeIgnored((HttpServletRequest)request)) {
			...
			requestService.startRequest(request, response); // will trigger the
															// various request
															// listeners
			try {
			 ...
			 * "Failure in the DSpaceWebappServletFilter: " + e.getMessage();
			 * System.err.println(message); if (res != null) {
			 * res.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
			 * message); } else { throw new ServletException(message, e); }
			 */
			// }
		}
	}
	...
	public DSpaceKernel getKernel() {
		...
		if (!kernel.isRunning()) {
			throw new IllegalStateException(
					"The DSpace kernel is not running: " + kernel);
		}
		return kernel;
	}
